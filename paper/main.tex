...
I would also like to acknowledge help received from RNDr. JUDr. Pavol Sokol, PhD. on legal questions, and access to resources from prof. RNDr. Gabriel Semanišin, PhD.
...
The result is an open software platform for operating systems Windows, Mac OS X, and Linux.
...
Výsledkom je otvorená softvérová platforma pre operačné systémy Windows, Mac OS X a Linux.
...
In Slovakia, the private key is usually handed over to the user personally, saved on the ID card. This ID card (used together with a card reader) provides an \pojem{Application Programming Inteface} (API) for safe access, management, and cryptoprocessing.
...
On the other thand, it lacks support for platforms other than Windows.
...
Signing modules - backends - are separate \pojem{Command Line Interface} (CLI) applications written in any programming language.
...
The backend that we call \pojem{Advanced Electronic Signature} (AES), is a Java CLI application that uses \pojem{Digital Signature Service} (DSS) framework. 
...
From the technological point of view, the website is a \pojem{Single Page Application} (SPA) written in React\footnote{React is a JavaScript library for building user interfaces, see \url{https://reactjs.org/}.}.
